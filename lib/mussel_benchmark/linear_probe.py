...
from .utils import _safe, extract_metrics
...
def parse_triples(args):
    """Parse 'model:json:csv' strings into (model, json_path, csv_path) tuples."""
    triples = []
    for arg in args:
        parts = arg.split(":", 2)
        if len(parts) != 3:
...
def get_json_for_model(model, triples):
    """Return parsed results.json for *model*, raising ValueError if absent."""
...
def build_summary(triples):
    """Parse all triples and return (summary_rows, clf_dfs)."""
...
def write_summary_csv_png(summary_rows):
    """Write summary.csv + summary.png; returns sorted DataFrame."""
...
def write_per_class(models, clf_dfs):
    """Write per_class_f1.csv + per_class_heatmap.png; returns (rows, class_cols)."""
    class_cols = None
    per_class_rows = []
...
def write_precision_delta(models, summary_rows, clf_dfs):
    """Write precision_delta.csv; returns DataFrame."""
    delta_rows = []
    processed_bases = set()
...
def summarize(triples):
    """Run the full linear-probe summarisation pipeline.

    Parameters
    ----------
    triples : list of (model_name, json_path, csv_path) tuples
    """
...
def main(argv=None):
    """CLI entry point: model1:results.json:clf_report.csv ..."""
    args = (argv if argv is not None else sys.argv[1:])
    if not args:
        print("Usage: summarize_linear_probe.py model:results.json:clf_report.csv ...", file=sys.stderr)
        sys.exit(1)
    summarize(parse_triples(args))
